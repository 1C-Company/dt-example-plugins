 ...
 * В данном диалоге пользователь может выбрать регистр накопления, для которого он хочет сделать обработку проведения. А так же указать, 
 * состав полей, которые будут учавствовать в логике работы обработчика <br>
    ...
    extends TitleAreaDialog
{
     ...
     */
    public DataProcessingHandlerDialog(Shell parentShell, Document document,
        List<BasicRegister> registers)
    {
        super(parentShell);
     ...
     * @return набор выражения обработчика проведения. Данный набор содержит пары значений. Где первый элемент пары - это левая часть выражения, которое
     * нужно добавить в тело обработчика, а второй элемент пары - это правая часть выражения. Не может быть <code>null</code>
     */
     ...
     * В нем происходит наполнение набора выражений обработчика проведения, в соответствии с теми данными, которые указал пользователь при работе с диалогом
     */
    @Override
    ...
    private void createTableViewer(Composite control)
    {
        ...
        registerTable =
            createDataTable(composite,
                Messages.DataProcessingHandlerDialog_Registers, getRegisters());
            ...
            public void selectionChanged(SelectionChangedEvent event)
            {
                ...
                if (!registerName.equals(((IStructuredSelection)event.getSelection()).getFirstElement()))
                {
                    ...
                    List<Expression> expressions =
                        createExpressionContent(index);
                    expressionTable.setInput(expressions);
                    expressionTable.refresh();
                }
            }
        });
        final TableViewer documentTable =
            createDataTable(composite,
                Messages.DataProcessingHandlerDialog_Document_attributes,
        ...
        documentTable.addDoubleClickListener(new IDoubleClickListener()
        {
            ...
            public void doubleClick(DoubleClickEvent event)
            {
                ...
                //при данном событии мы заполняем правую часть активного выражения обработчика проведения именем данного атрибута
                int index = documentTable.getTable().getSelectionIndex();
                String documentAttr =
                    ((List<String>)documentTable.getInput()).get(index);
                int expressionIndex =
                    expressionTable.getTable().getSelectionIndex();
                List<Expression> listOfExpressions =
                    (List<Expression>)expressionTable.getInput();
                if (expressionIndex < 0
                    || expressionIndex >= listOfExpressions.size())
                {
                    expressionIndex = 0;
                }
                listOfExpressions.get(expressionIndex).expression =
                    documentAttr;
                expressionTable.refresh();
            }
        });
    ...
    private TableViewer createExpressionTable(Composite composite,
        TableColumnLayout columnLayout)
    {
        final TableViewer tableViewer =
            new TableViewer(composite, SWT.BORDER | SWT.V_SCROLL
                | SWT.FULL_SELECTION);
        GridDataFactory.fillDefaults().grab(true, true).applyTo(
            tableViewer.getTable());
        int height = tableViewer.getTable().getItemHeight() * 5;
        tableViewer.getTable().setHeaderVisible(true);
        GridDataFactory.fillDefaults().hint(SWT.DEFAULT, height).grab(true,
            false).applyTo(tableViewer.getTable());
        TableViewerColumn column =
            new TableViewerColumn(tableViewer, SWT.BORDER);
        columnLayout.setColumnData(column.getColumn(), new ColumnWeightData(5,
        ...
        column.setLabelProvider(new DelegatingStyledCellLabelProvider(
            new ExpressionLabelProvider(true)));
        column = new TableViewerColumn(tableViewer, SWT.BORDER);
        columnLayout.setColumnData(column.getColumn(), new ColumnWeightData(5,
            ColumnWeightData.MINIMUM_WIDTH, true));
        column.getColumn().setResizable(true);
        column.getColumn().setText(
            Messages.DataProcessingHandlerDialog_Expressions);
        column.setLabelProvider(new DelegatingStyledCellLabelProvider(
            new ExpressionLabelProvider(false)));
    ...
    private List<Expression> createExpressionContent(int index)
    {
        ...
        List<String> registerAttr =
 getRegisterAttr(registers.get(index));
    ...
    private TableViewer createDataTable(Composite composite, String headerName,
        List<String> content)
    {
        final TableViewer tableViewer =
            new TableViewer(composite, SWT.BORDER | SWT.V_SCROLL
    ...
    private List<String> getDocumentAttributes()
    {
        ...
        for (DocumentAttribute attr : document.getAttributes())
        {
            if (!Strings.isNullOrEmpty(attr.getName()))
                registers.add(attr.getName());
        }
        for (Field field : document.getFields())
        {
            if (!Strings.isNullOrEmpty(field.getNameRu()))
                registers.add(field.getNameRu());
        }
        return registers;
    }
        ...
        public StyledString getStyledText(Object element)
        {
            if (isFieldProvider)
                return new StyledString(
                    super.getText(((Expression)element).field));
            else
                return new StyledString(
                    super.getText(((Expression)element).expression));
        }
